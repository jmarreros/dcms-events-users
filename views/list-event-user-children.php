...
<?php if ( count( $children ) == 0 ) : // Not children yet ?>
        ...
        <?php if ( ! $lock_inscriptions && ! $is_selected ): ?>
        <section class="container-question">
            <label>
                <input type="checkbox" class="question-children" name="question-<?= $id_event ?>"> Quiero agregar
                abonado acompañante
            </label>
        </section>
    <?php endif; ?>
...
<?php else : // Has children ?>
    ...
    <?php if ( ! $lock_inscriptions && ! $is_selected ): ?>
        <section class="container-question no-mark">
            <label>
                <input type="checkbox" class="question-children" name="question-<?= $id_event ?>"> Editar abonado
                acompañante
            </label>
        </section>
    <?php endif; ?>
...
<?php if ( ! $lock_inscriptions && !$is_selected) : // lock_inscriptions?>
